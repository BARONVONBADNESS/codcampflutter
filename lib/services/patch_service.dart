...
class PatchService {
  ...
  static PatchIntelItem _mapToPatchItem(Map<String, dynamic> p) {
    ...
    final messageId = (p['id'] as String? ?? '').replaceFirst('discord_', '');
    final timestamp = DateTime.tryParse(p['timestamp'] as String? ?? '')
  ...
  static String _extractIssueCode(List<String> lines, DateTime timestamp) {
    ...
    if (lines.isNotEmpty) {
      ...
      if (parts.length >= 2) {
        ...
        patchName = parts[0].replaceAll(RegExp(r'^[\s\S]*?(?=[A-Za-z])'), '').trim();
        patchDate = parts[1].trim();
      } else {
        // No separator — try to extract inline date
        final dateMatch = RegExp(r'([A-Za-z]+ \d{1,2},?\s*\d{4})').firstMatch(clean);
        if (dateMatch != null) {
          patchDate = dateMatch.group(1) ?? '';
          patchName = clean.replaceAll(dateMatch.group(0) ?? '', '').replaceAll(RegExp(r'[^\x00-\x7F]'), '').trim();
        } else {
    ...
    if (patchDate.isEmpty) {
      ...
      patchDate = '${months[timestamp.month - 1]} ${timestamp.day}, ${timestamp.year}';
    }
  ...
  static String _detectType(String content) {
    ...
    if (s.contains('weapon') || s.contains('dmg') || s.contains('damage') ||
        s.contains('nerf') || s.contains('buff') || s.contains('recoil')) return 'Weapons';
    if (s.contains('ranked') || s.contains('rank')) return 'Ranked';
    if (s.contains('season') || s.contains('reloaded') || s.contains('update') ||
        s.contains('patch') || s.contains('summary')) return 'Summary';
  ...
  static String _detectImpact(String content) {
    final s = content.toLowerCase();
    if (s.contains('major') || s.contains('significant') || s.contains('season') ||
        s.contains('reloaded') || s.contains('big') || s.contains('massive')) return 'High';
    if (s.contains('minor') || s.contains('small') || s.contains('slight')) return 'Low';
    return 'Medium';
  }

  static IconData _detectIcon(String content) {
    final s = content.toLowerCase();
    if (s.contains('weapon') || s.contains('gun') || s.contains('damage')) return Icons.track_changes_rounded;
    if (s.contains('season') || s.contains('reloaded')) return Icons.article_rounded;
    if (s.contains('ranked')) return Icons.emoji_events_rounded;
    if (s.contains('monitor') || s.contains('track')) return Icons.radar_rounded;
  ...
  static List<String> _extractTags(String content, String type) {
    ...
    if (s.contains('multiplayer') || s.contains(' mp ')) tags.add('Multiplayer');
